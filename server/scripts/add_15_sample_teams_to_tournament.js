...
async function seedData() {
    try {
        ...
        if (!tournament) {
            console.error('❌ Tournament not found with ID:', TOURNAMENT_ID);
            process.exit(1);
        }

        console.log(`🚀 Starting simulation for tournament: ${tournament.tournamentName}`);
        
        const gameName = tournament.game || 'BGMI'; // default to BGMI if not set
        ...
        console.log(`\n✨ Simulation complete! ${TEAM_COUNT} teams created and registered.`);
        process.exit(0);
    } catch (error) {
