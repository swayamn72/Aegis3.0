...
const TeamGrouping = ({ tournament, onUpdate }) => {
    ...
    const getPhaseTeams = () => {
        const phase = tournament.phases?.find(p => p.name === selectedPhase);
        // If phase.teams is an array of ObjectIds, you may need to populate team data elsewhere
        return phase?.teams?.map(team => {
            if (typeof team === 'object' && team !== null) {
                ...
                // fallback if only ObjectId is present
                return {
                    _id: team,
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            await axiosInstance.put(
                `/api/tournaments/${tournament._id}/groups`,
                {
                    groups: groups,
                    phaseId: phase._id
                }
            ...
            toast.error(error?.error || error?.message || 'Failed to save groups');
        } finally {
            setLoading(false);
        }
    };
