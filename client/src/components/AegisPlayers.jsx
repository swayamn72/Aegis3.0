...
import AegisProfileCardBGMI from './profileCards/AegisProfileCardBGMI';
import { useAuth } from '../context/AuthContext';
import { usePlayersQuery } from '../hooks/usePlayersQuery';

// Move FilterDropdown outside and memoize
...
const PlayerCard = React.memo(({ player, onViewProfile }) => {
        ...
        <div className="bg-zinc-950 border border-zinc-900 rounded-lg overflow-hidden transition-all duration-300 hover:border-zinc-800 hover:shadow-2xl hover:shadow-black/50 group">
            <div className="relative p-5">
                ...
                <div className="flex gap-2">
                    <button
                        onClick={() => onViewProfile(player._id)}
...
const AegisPlayers = () => {
    ...
    const [filters, setFilters] = useState({ availability: '', status: '' });
    ...
    // ✅ Flatten all pages into single players array
    ...
    const handleViewProfile = useCallback((playerId) => {
        if (playerId) {
            navigate(`/detailed/${playerId}`);
        } else {
            console.error('Player _id is undefined');
        }
    }, [navigate]);

    // Optimize filtered players
    ...
    }, [searchTerm, filters, allPlayers, user, userType]);
    ...
    if (error) {
        return (
            <div className="min-h-screen bg-black text-white flex items-center justify-center">
                <div className="text-center">
                    <p className="text-red-400 text-lg mb-4">Failed to load players</p>
            ...
            <div className="relative z-10 max-w-[1400px] mx-auto px-6">
                ...
                <div className="mb-8 p-5 bg-zinc-950 border border-zinc-900 rounded-lg">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                        ...
                        <FilterDropdown
                            options={['All', 'weekends only', 'evenings', 'flexible', 'full time']}
                        ...
                        />
                        <FilterDropdown
                            options={['All', 'looking for a team', 'in a team', 'open for offers']}
                ...
                {/* Load More Section */}
                {filteredPlayers.length > 0 && hasNextPage && (
                    <div className="text-center mt-12">
                        <button
                            onClick={loadMorePlayers}
                            disabled={isFetchingNextPage}
                            className="bg-[#FF4500] hover:bg-[#FF4500]/90 disabled:bg-zinc-600 disabled:cursor-not-allowed text-white font-semibold px-8 py-3 rounded-lg transition-all"
                        >
                            {isFetchingNextPage ? (
                ...
                {filteredPlayers.length > 0 && !hasNextPage && (
                    <div className="text-center mt-12">
                        ...
                        <p className="text-zinc-600 text-sm mt-2">
                            Showing all {filteredPlayers.length} players
                        </p>
                    </div>
                )}
            </div>
