...
// Extract theme colors outside component - only created once
const THEME_COLORS = {
  orange: {
...
// Extract AegisMascot as separate memoized component
const AegisMascot = React.memo(({ colors }) => (
...
const AegisProfileCardBGMI = ({ cardTheme = 'orange', playerData }) => {
  ...
  const theme = useMemo(() => cardTheme || data.cardTheme || 'orange', [cardTheme, data.cardTheme]);
  const colors = useMemo(() => THEME_COLORS[theme] || THEME_COLORS.orange, [theme]);
...
export default React.memo(AegisProfileCardBGMI);
