...
const Tournaments = () => {
  ...
  // Use TanStack Query for tournaments
  const {
    ...
    queryKey: ['tournaments', filters], // filters in key for future extensibility
  ...
  const tournaments = useMemo(() => {
    ...
    if (Array.isArray(tournamentsData)) return tournamentsData;
    return tournamentsData.tournaments || [];
  }, [tournamentsData]);

  const liveTournaments = useMemo(() => {
    if (!tournamentsData) return [];
    if (Array.isArray(tournamentsData)) return tournamentsData.filter(t => t.status === 'in_progress');
    return tournamentsData.liveTournaments || [];
  }, [tournamentsData]);

  const upcomingTournaments = useMemo(() => {
    if (!tournamentsData) return [];
    if (Array.isArray(tournamentsData)) return tournamentsData.filter(t => t.status === 'announced' || t.status === 'registration_open');
    return tournamentsData.upcomingTournaments || [];
  }, [tournamentsData]);
      ...
      <div className="bg-gradient-to-br from-zinc-950 via-stone-950 to-neutral-950 min-h-screen text-white font-sans mt-20">
        <div className="container mx-auto px-6 py-12">
          <div className="text-center">
            ...
            <p className="text-zinc-400 text-lg mb-6">{error?.message || 'Failed to load tournaments. Please try again later.'}</p>
            <button
